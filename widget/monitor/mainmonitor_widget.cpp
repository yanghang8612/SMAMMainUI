...
//                    bool isConnected = false;
//                    for (int j = 0; j < IGMASSTATION_MAXITEMCOUNT; j++) {
...
void MainMonitorWidget::updateXJView()
{
    ...
    MainCenterNode* mainCenterNode = new MainCenterNode(mainCenter, 50);
...
//    scene->addItem(new Edge(fileNode, hardDriveNode));
    ...
    for (int i = 0; i < centerCount; i++) {
        ...
        OtherCenterNode* otherCenterNode = new OtherCenterNode(center, 50);
...
//    scene->addItem(new Edge(fileNode, hardDriveNode));
    ...
    QPointF startStationPoint(-550, - stationLength / 2 * (((stationCount > 6) ? 6 : stationCount) - 1));
    for (int i = 0; i < stationCount; i++) {
